...
class CircularClip extends React.Component {
    constructor(props) {
        ...
        this.path = (start, end, ease) => {
            ...
            if (click < 1) {
                newX = radious * (lerp_1.default(1, 0, ease));
                newY = Math.sin(lerp_1.default(100, 0, ease) / 20) * y + 30;
            }
            else {
                newX = radious * (lerp_1.default(0, 1, ease));
                newY = Math.sin(lerp_1.default(0, 100, ease) / 20) * y;
            }
